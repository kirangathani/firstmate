...
Report only true captain-relevant outcomes or a declared external wait by appending one line:
   \`echo "[t=\$(date +%s)] {state}: {one short line}" >> $STATUS_FILE\`
   The \`[t=...]\` stamp is the epoch second you reported at. It is what lets firstmate measure how long you waited for an answer, so keep it; a line without one is still understood, it just cannot be timed.
...
9. Run the repository's lint ONCE, on the whole tree, at the end - immediately before the commit you
   push - never after each edit and never in a loop. A full pass can be 20+ concurrent linter
   processes and many gigabytes of memory, and simultaneous passes have taken the whole machine down
   under the fleet. While editing, run the linter on just the file you changed instead.

# Project memory
If \`AGENTS.md\` already exists, or if this task produced durable project-intrinsic knowledge, run \`$FM_ROOT/bin/fm-ensure-agents-md.sh .\` in the worktree.
