...
rm -f "$STATE/$ID.status" "$STATE/$ID.turn-ended" "$STATE/$ID.meta" "$STATE/$ID.pi-ext.ts" "$STATE/$ID.grok-turnend-token" "$STATE/$ID.acted" "$STATE/$ID.stale-base-ack" "$STATE/$ID.monitor-exempt" "$STATE/.unactioned-$ID" "$STATE/$ID.nm-progress" "$STATE/$ID.nm-stall-ack" "$STATE/$ID.merge-green-rounds" "$STATE/$ID.nm-attach"
