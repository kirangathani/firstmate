...
fm_super_main() {
  ...
  # backend=tmux this resolves $TARGET with `tmux list-panes -t "$TARGET"` and
  # passes no expected label, so the supervisor pane keeps tmux's own target
  # resolution (contract: fm_backend_tmux_target_exists, bin/backends/tmux.sh).
