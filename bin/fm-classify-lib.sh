...
# own records can say how long it took to answer a crew; docs/configuration.md's
# "Self-latency ledger" section is what reads it. It sits BEFORE the verb rather
# than after the note because the note is the only free-text cell on the line,
# and a trailing field would have to be delimited out of arbitrary prose; epoch
# seconds carry no colon, so a leading token also cannot disturb the
# "<verb>: <note>" split below.
#
