...
4. If the crewmate is genuinely wedged after redirection, exit the agent with the adapter's exit command and relaunch with the same brief plus a `progress so far` note appended to it.
   ...
   If a high context reading is paired with uncommitted changes in the task worktree, steer the crewmate to commit them before any interrupt, exit, or relaunch; `AGENTS.md` section 8 owns that carve-out.
