...
- `NO_MISTAKES_DAEMON: not running (restart: no-mistakes daemon start)` - the shared no-mistakes daemon (one instance serving every lane and home) is installed but dead, most often after a machine reboot; every ship task's validation pipeline depends on it.
  Tell the captain plainly that pipeline validation will fail until it is restarted, and offer to run the printed `no-mistakes daemon start` yourself; do not run it without the captain's go-ahead, since restarting can race a lane that started the daemon back up between the probe and your action.
  Never run `no-mistakes daemon stop` or `restart` speculatively - stopping the shared daemon kills every other lane's in-flight pipeline run, per `AGENTS.md` section 1 rule 3 and rule 7 in every crewmate brief.
