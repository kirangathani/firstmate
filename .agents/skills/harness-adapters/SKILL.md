...
Like every other send, issue it as its own Monitor rather than a shell command: the raw window target and `--key` behave exactly the same there, and the Bash tool refuses it.
