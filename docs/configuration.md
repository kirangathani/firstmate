...
It does not make several green PRs of one project land at once: `bin/fm-merge-green.sh` lands them one at a time, oldest dispatch first, and steers only the branch next to land when main has moved, which its header owns.
...
The regions are delimited in the brief by `<!-- fm:... -->` markers, and the two axes they are a function of are recorded on their own markers above the definition of done - the delivery mode on `<!-- fm:delivery-mode ... -->` and the skip on the definition-of-done marker - so an apply that would change nothing rewrites nothing and an apply that changes either one does; `bin/fm-brief.sh`'s header owns them.
...
FM_NM_STALL_NOW=      # test hook: freeze the clock bin/fm-nm-stall.sh measures observation spans against
FM_PR_GH_TIMEOUT=15     # seconds bounding fm-pr-lib.sh's GitHub base-branch read, the one owner of which branch a PR targets
