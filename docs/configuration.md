...
FM_NM_STALL_SECS=10800  # seconds a no-mistakes step may go unchanged across observations before bin/fm-nm-stall.sh reports it as stopped advancing; that script's header justifies the default against measured step durations
...
FM_NM_STALL_MAX_READS=4   # tasks read per stalled-validation sweep, least-recently-observed first, so a large fleet spreads across sweeps
