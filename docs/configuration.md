...
It also wakes firstmate only once the task's own last status event is a `done:` line, because a worker mid-run can have a head GitHub already calls green, and `bin/fm-pr-poll.sh`'s header owns that rule.
