...
Arming itself is gated on the home's session lock, `state/.lock`, inside `bin/fm-watch-arm.sh`, so every harness inherits one rule: a session that does not hold that lock declines quietly with exit 0 instead of taking the watcher singleton for a fleet it is not responsible for.
