...
No-verb wakes, such as `working:` notes and bare turn-ended signals, are benign only when `bin/fm-crew-state.sh` reports positive evidence that the crew is still working: an actively running no-mistakes step attributed to that crew's own ship branch `fm/<id>`, or a backend busy signature.
A crew that declares `paused:` for a known external wait is separately absorbed while idle and re-surfaced only on the longer pause cadence, rather than being treated as a possible wedge.
For an ordinary crew whose backend confidently reports its agent dead, that cadence applies to a `paused:` or durable `captain-held` endpoint immediately; a live or inconclusively read agent instead surfaces one stale wake first.
That first check is once per declaration, not once per pane hash, so an idle pane whose capture keeps changing on its own cannot turn it into a repeating stale wake, and the secondmate idle-endpoint exemption is unchanged.
...
`bin/fm-crew-state.sh <id>` is the cheap current-state read for an actionable heartbeat review: it attributes the matching no-mistakes run, active or terminal, to the crew's own ship branch `fm/<id>` rather than to whatever the recorded worktree currently has checked out, and keeps that run-step authoritative even if the pane has closed.
...
That `fm/<id>` key exists because a pooled worktree slot is released when its worker dies without teardown while `state/<id>.meta` deliberately survives, so once the slot is re-leased anything read through the recorded path - `git symbolic-ref` and an in-worktree `axi status` alike - answers for the newer live occupant.
The recorded checkout is therefore only an ownership signal: still on `fm/<id>` keeps full attribution, a worktree provably another task's is read only through the repo-wide `no-mistakes runs` list for `fm/<id>`, and an off-contract checkout that is neither keeps its in-worktree answer only while this crew's own agent process is confirmed alive.
Where ownership stays unproven, all three healthy-verdict sites - an active run record, a coarse `running` row, and a busy pane - report `unknown` so the crew surfaces for supervision instead of reading as working, and every verdict carries the `worktree not on fm/<id>` observation that recovery needs.
The script header owns the exact tiers and mappings.
...
On `attached` it stays live across identity-matched successors, and a clean child close either attaches to a verified healthy successor or is classified by `cycle_outcome`: a cycle that delivered a wake reports `watcher: cycle-complete` and exits 0, a stale beacon past the grace reports the typed nonzero `watcher: FAILED - supervision LAPSED`, and a close with neither keeps the typed nonzero `watcher: FAILED - cycle ended without an actionable reason` (`docs/watcher-continuity.md` owns the contract).
...
Arming itself is gated on the home's session lock, `state/.lock`, at both entry points that take the watcher singleton: `bin/fm-watch-arm.sh` and Codex's `bin/fm-watch-checkpoint.sh`.
Every harness therefore inherits one rule: a session that does not hold that lock declines quietly with exit 0 instead of taking the watcher singleton for a fleet it is not responsible for.
The gate lives on those entry points rather than on `bin/fm-watch.sh` itself, which is also forked as a child by the arm and by the away-mode daemon and must never refuse a legitimate child.
`bin/fm-session-lock-lib.sh` is the single implementation of that ownership walk, and Pi and OpenCode reach it through `bin/fm-lock.sh ownership` rather than reimplementing it.
It identifies the lock holder by PID and, on Linux, by the kernel's process start ticks, so a PID reused after a reboot reads as a stale lock rather than a live rival: the home arms with its announced notice and the blind-turn alarm still fires.
The holder it records is the acquiring session's OWN nearest harness process, found through the pid marker that harness advertises and otherwise by an ancestry walk that recognises a versioned install layout as well as a command name, so ownership does not rest on a process the harness itself replaces: a background Claude Code session whose lock named the interactive session above it lost supervision of its fleet for eighteen minutes when the daemon between the two self-restarted for an auto-update (the 2026-09-15 lock-loss incident).
A live holder that is an ANCESTOR of the acquiring session is INHERITED rather than refused, at session start and again on the owned path of both arming entry points, so the first session forked from the holder to act becomes the sole owner, a lock naming an ancestor migrates with no operator step, and the ancestor then correctly reports that it is not in control.
Only a live holder outside that ancestry is a rival, and it still refuses.
...
The deeper agent-process liveness probe is separate from that busy-state poll: tmux and herdr have verified classifiers, used by session-start secondmate recovery and by `fm-crew-state.sh`'s worktree-ownership gate, while zellij, Orca, and cmux currently report `unknown` rather than guess.
Because that gate treats only a literal `alive` as proof, an unclassifiable backend simply keeps a crew off the contract branch reading `unknown` instead of `working`.
...
`bin/fm-pr-merge.sh`'s header owns everything on the consuming side: the four blocking finding classes, the per-project `data/exec-gate/<project>` marker that decides whether an unexecuted finding blocks or only warns, the refuse/proceed decision rule across all four classes, and the supersession entry grammar.
The same base assertions are re-run in CI by the required `Base assertions re-verified` check, which cannot read that private approval record and therefore stays red on an approved supersession, so the merge gate excuses that one further check from its own local run of those same assertions rather than sending the decision back through CI.
Publishing a captain-signed attestation to turn that check itself green is optional and changes no merge verdict; `bin/fm-supersession-attest-lib.sh` owns its wire and docs/configuration.md "The supersession attestation secret" owns the provisioning.
One of those four, `unstable:`, is the gate refusing to compare an assertion whose own name changed between check 2's two runs of the identical base file, which is a defect in that test rather than in the branch under review.
After the test-keep gate it also runs a checks-green gate against the PR's current check rollup, refusing a failing check (red), a pending check (distinctly - the remedy is waiting, not fixing), an unclassifiable or unreadable check state (unverified), and a PR reporting zero checks unless a captain's decision says that absence was deliberate - either the per-project `data/no-pr-ci/<project>` marker or a signed `ci_skip` on that task; the same header owns the classification table and the zero-checks contract.
...
Durable project-intrinsic agent knowledge lives in each project's committed `AGENTS.md`, the only memory file the convention keeps.
...
It never creates a `CLAUDE.md`, and deletes one that is only a symlink to `AGENTS.md` or a bare `@AGENTS.md` shim, so a converted project stays converted.
It refuses a case-variant real memory file such as a lowercase `agents.md`, which satisfies every `AGENTS.md` existence test on a case-insensitive filesystem but is a different path on a case-sensitive one, and surfaces the mismatch for manual reconciliation.
