...
For an ordinary crew whose backend confidently reports its agent dead, that cadence applies to a `paused:` or durable `captain-held` endpoint immediately; a live or inconclusively read agent instead surfaces one stale wake first, labelled as a declared-pause first check so it is not read as a wedge.
