...
`bin/fm-stale-base.sh` ranks each project's branches into the landing queue `bin/fm-merge-green.sh` lands in, from the order's one owner `bin/fm-landing-queue-lib.sh`, and reports the head of that queue as `STALE BASE:`, which blocks here, and every other behind branch as `PARKED BASE:`, which does not and asks for nothing.
The ranking covers every branch of the project that carries a PR, not only the behind ones, because a branch that has already merged the base forward and is waiting on its checks still holds the head of the queue.
While the head is caught up like that, no branch is `STALE BASE:` at all: everything behind it is parked, nothing blocks here, and nobody is steered until it lands.
