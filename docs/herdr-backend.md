...
Each of those four was confirmed to FAIL against its adapter's pre-fix version with exactly the reported defect (the blank-padded row reading `unknown`).
`tests/fm-daemon.test.sh` stays green, so no adapter regressed.
