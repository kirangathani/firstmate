...
| Send + submit atomically | `herdr pane run <pane> <command>` | Runs and submits a command in one call; used for the fixed spawn-time commands (`treehouse get`, the `GOTMPDIR` export, and a `--local-skip` task's shim `PATH` export) exactly where tmux used one `send-keys ... Enter` call. |
...
**The adapters' own PRE-classifier trims, closed as the follow-up to the above.** The same four adapters each ran their own `[[:space:]]` trims to decide row shape and to strip box borders, and those run BEFORE the classifier, so a row whose blank padding sits OUTSIDE the shape being matched (an NBSP indent before a border glyph or before a bare prompt glyph) failed the adapter's structural test and never reached the classifier that would have normalized it.
All of those sites now route through the same shared `fm_composer_normalize_trim` instead of a bare `[[:space:]]` strip: `fm_tmux_composer_state` (`bin/fm-tmux-lib.sh`, plain-row border match plus both border strips), `fm_backend_herdr_composer_state` and `fm_backend_herdr_pi_separator_row` (`bin/backends/herdr.sh`), `fm_backend_orca_composer_state` (`bin/backends/orca.sh`) and `fm_backend_cmux_composer_state` (`bin/backends/cmux.sh`).
Extending the existing shared owner, rather than open-coding a second blanks table per adapter, is what `fm_composer_normalize_trim` was documented as idempotent and safe for.
Each site was checked individually rather than substituted mechanically: the border strips target box glyphs, not whitespace, so normalizing the padding cannot change what they match, and `fm_backend_herdr_pi_separator_row`'s `>= 8` length threshold now measures the `─` rule itself rather than its padding, which is the intended semantics.
The zero-width/format exclusion above is unchanged, so U+2063 still counts as content and the safe bias holds.
...
The adapters' own PRE-classifier trims are pinned by one colocated test per adapter, each looping over every `FM_COMPOSER_BLANKS` character and asserting the verdict is identical to the ASCII-space-padded twin fixture beside it: `test_composer_state_blank_padded_rows_match_ascii` in `tests/fm-backend-herdr.test.sh` (blank-padded bordered box -> `empty`, blank-padded bare `❯` -> `empty`, real text on a blank-padded row -> `pending`, and a blank-padded solid `─` rule still reading as a Pi separator row), the same-named tests in `tests/fm-backend-orca.test.sh` and `tests/fm-backend-cmux.test.sh` (blank-padded bordered box -> `empty`, real text -> `pending`), and `test_tmux_reader_blank_padded_border_rows_match_ascii` in `tests/fm-send-blank-composer.test.sh` for `fm_tmux_composer_state`'s bordered rows.
Each of those four was confirmed to FAIL against its adapter's pre-fix version, in the two distinct modes the pre-fix code produces.
The herdr, orca and cmux tests fail as `unknown`: the surviving leading blank makes the blank-padded row match neither the bordered `'│'*'│'` shape nor herdr's anchored `^[❯›]` bare-prompt regex, so the structural scan finds no row at all (`found=0`) and returns `unknown` before the classifier is reached.
The tmux test fails as `pending`: `fm_tmux_composer_state` scans no rows, so the surviving leading blank instead leaves `bordered=0` and skips the border strip, and the whole un-stripped `<blank>│ ❯ <blank>│<blank>` row is handed to `fm_composer_classify_content`, which normalizes it, finds it non-empty, matches no glyph and no idle pattern, and falls through to `pending` (`bin/fm-composer-lib.sh`).
That second mode is the same false-`pending` direction this whole section documents.
