...
data/                personal fleet records; LOCAL, gitignored as a whole
  ...
  timeline.tsv       append-only task timeline ledger, one line per finished ship task from dispatch to merged PR; LOCAL, gitignored, written by bin/fm-timeline.sh (which owns its columns) as bin/fm-teardown.sh's first step, read back with `bin/fm-timeline.sh report`
