...
The `PR must be raised via no-mistakes` check is red by design on a direct-PR project's PR and on a task carrying a signed testing skip, so that command excuses it through the same owner `bin/fm-pr-merge.sh` excuses it through and names the reason on its green line; both authorities are read from the task's own record, so it must be run against the home that dispatched the task and it says so rather than reporting a verdict when it cannot read one.
