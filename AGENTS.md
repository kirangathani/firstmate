...
That ownership is a liveness obligation, not only an authority: one attach covers one return, nothing is attached once that line is appended, and a run parked with no live attach is a stalled run that nothing will resume on its own.
